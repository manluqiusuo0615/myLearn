...
注解如同标签,标签是对事物行为的某些角度的评价与解释   
代码具有生命,注解就是对于代码中某些鲜活个体贴上去的一张标签。简化来讲,注解如同一张标签。   
Annotation(注解)是JDK1.5及以后版本引入的。它可以用于创建文档，跟踪代码中的依赖性，甚至执行基本编译时检查。  
同classs和interface一样,注解也属于一种类型。  
...
以简单理解为创建了一张名字为 TestAnnotation 的标签。   
## 注解的应用
创建了一个注解，那么注解的的使用方法是什么呢。  
```
@TestAnnotation
public class Test {
}
...
### @Inherited
Inherited 是继承的意思，但是它并不是说注解本身可以继承，而是说如果一个超类被 @Inherited 注解过的注解   
进行注解的话，那么如果它的子类没有被任何注解应用的话，那么这个子类就继承了超类的注解。   
```
@Inherited
@Retention(RetentionPolicy.RUNTIME)
@interface Test {}
//一个Test注解被  @Inherited  @Retention  注解了
...
对于 @Person(role=”PM”) 括号里面的内容   
其实就是给 Person 这个注解的 role 属性赋值为 PM    
## 注解的属性
...
它的存在会阻止编译器产生 unchecked 这样的警告。它是在 Java 1.7 的版本中加入的。   
```$xslt
...
## 注解的提取
用标签来比作注解，前面的内容是讲怎么写注解，然后贴到哪个地方去，而现在要做的工作就是检阅这些标签内容。  
...
如果某个注解没有@Retention(RetentionPolicy.RUNTIME)  是无法获取到的。
比如：check上面没有@Retention(RetentionPolicy.RUNTIME)，那么久无法获取  check value:hi
## 注解的使用场景
注解到底有什么用？  
Java 官方文档:  
...
**值得注意的是，注解不是代码本身的一部分。**   
如果难于理解，可以这样看。罗永浩还是罗永浩，不会因为某些人对于他“傻x”的评价而改变，标签只是某些人对于其他事物的评价，  
但是标签不会改变事物本身，标签只是特定人群的手段。所以，注解同样无法改变代码本身，注解只是某些工具的的工具。  
...
```
编写一个测试类 TestTool 就可以测试 NoBug 相应的方法了。
```$xslt
...
所以，NoBug 这个类有 Bug。   
这样，通过注解完成了自己的目的，那就是对别人的代码进行测试。  
再问注解什么时候用？这取决于你想利用它干什么用。  
## 总结
